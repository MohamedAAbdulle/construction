...
const InvCorrection = ({ open, onClose, inv }) => {
  ...
  const onAddMore = () => {
    putEndpoint(`/inventory/quantity/${inv.id}`, state).then(() => {
      onClose();
      getInvList(true);
    });
  ...
  return (
    <ModalCont open={open} onClose={onClose} title={`Correction (${inv.name})`}>
      <div className="row gy-3">
        ...
        <div className="col-12 col-lg-7">
          <div className="d-flex align-items-center">
            <h3 className="fw-normal my-0">{inv.quantity}</h3>
